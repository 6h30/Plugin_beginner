...
Bước 1: Các loại Tag trong Revit APIRevit API cung cấp hai lớp chính để gắn thẻ:
...
Bước 2: Tham số của IndependentTag.CreatePhương thức IndependentTag.Create (phiên bản mặc định) yêu cầu:
...
Hiển thị mã giả lập:  IndependentTag tag = IndependentTag.Create(doc, viewId, reference, true, TagMode.TM_ADDBY_CATEGORY, TagOrientation.Horizontal, point);
...
Bước 3: Mở tệp TagView.csMở tệp TagView.cs trong dự án MyRevitCommands. Mã cơ bản đã được chuẩn bị, lấy Document và bắt đầu Transaction:
...
Bước 4: Định nghĩa TagMode và TagOrientationTạo các biến cho TagMode và TagOrientation:
...
Bước 5: Lọc cửa ra vào và cửa sổSử dụng ElementMulticategoryFilter để lấy các phần tử Doors và Windows trong view hiện tại:
...
Bước 6: Gắn thẻ từng phần tửTrong Transaction, duyệt qua danh sách phần tử và gắn thẻ:
...
Bước 7: Chạy Debug và kiểm traTrong Visual Studio, nhấn F5 để chạy debug mode. Mở Revit, mở tệp bài tập của video này, chứa một số cửa ra vào và cửa sổ. Chạy Add-Ins > External Tools > TagView. Kết quả: Tất cả cửa ra vào và cửa sổ trong view hiện tại sẽ được gắn thẻ với leader, sử dụng TagMode ngang và theo danh mục.
Hành động trên màn hình:  
...
Bước 8: Kêu gọi hành độngHãy thử mở rộng lệnh để gắn thẻ trong nhiều view thay vì chỉ view hiện tại. Hoặc thử thay đổi TagMode thành TM_ADDBY_MATERIAL để gắn thẻ theo vật liệu. Nếu bạn gặp vấn đề, hãy để lại câu hỏi trong phần bình luận, mình sẽ hỗ trợ ngay!
